...
namespace Vouchee.Business.Services.Impls
{
    public class DiscountService : IDiscountService
    {
        ...
        public async Task<DynamicModelResponse.DynamicModelsResponse<DiscountResponse>> GetDiscounts(DiscountFilter request, PagingRequest paging)
        {
            ...
            try
            {
                var query = _discountRepo.GetAllAsync().Result;
                var filtercheck = request.GetType().GetProperties().All(p => p.GetValue(request) != null);
                var mappedRequest = filtercheck ? _mapper.Map<DiscountResponse>(request) : null;
                var filteredQuery = mappedRequest != null
                    ? query.ProjectTo<DiscountResponse>(_mapper.ConfigurationProvider).DynamicFilter(mappedRequest)
                    : query.ProjectTo<DiscountResponse>(_mapper.ConfigurationProvider);
